...
import { addUser } from "../store/actions/todoAction";
...
const Newlist = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    todos: [{ id: 1, status: "pending", todo: "" }],
    priority: "",
  });
  ...
  const handleAddTodo = () => {
    ...
    setFormData({
      ...
      todos: [
        ...formData.todos,
        { id: newTodoId, status: "pending", todo: "" },
      ],
    });
  };

  const handleDeleteTodo = (id) => {
    const updatedTodos = formData.todos.filter((todo) => todo.id !== id);
    setFormData({ ...formData, todos: updatedTodos });
  };
    ...
    <>
      <div className="flex pt-20 justify-center h-screen w-screen bg-purple-700">
        <div className=" scrollbar bg-customColor overflow-y-scroll max-h-[80%] p-8 rounded w-[90%]  md:w-[80%] lg:w-6/12 shadow-md">
          <h1 className="text-3xl text-white  font-bold mb-4">Add New Todo</h1>
